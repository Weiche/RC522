...
void RFID_init() {
	...
	RFID_reset();
	RFID_DelayMs(100);
	while (RFID_readMFRC522(CommandReg) & (1<<4));
...
void RFID_calculateCRC(unsigned char *pIndata, unsigned char len,
...
unsigned char RFID_MFRC522ToCard(unsigned char command, unsigned char *sendData,
	...
	RFID_writeMFRC522(CommIEnReg, irqEn | 0x80);//De solicitud de interrupci�n
	RFID_clearBitMask(CommIrqReg, 0x80);// Borrar todos los bits de petici�n de interrupci�n
	RFID_setBitMask(FIFOLevelReg, 0x80);//FlushBuffer=1, FIFO de inicializaci�n
 ...
 */
unsigned char RFID_auth(unsigned char authMode, unsigned char BlockAddr,
